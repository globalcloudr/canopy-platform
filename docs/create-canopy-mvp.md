# Create Canopy MVP Definition

Date: 2026-03-23

## Purpose

Define `Create Canopy` as an MVP product inside the Canopy platform so it can be designed and built without ambiguity.

## What Create Canopy Is

Create Canopy is the design request and creative services management product inside Canopy.
...
Create Canopy puts a formal process around creative services and creates a clear path for Canopy to charge for design work.

## Request Types

Create Canopy supports the design request types schools most commonly need:
...
1. School staff submits a design request through Create Canopy
...
9. On approval, the final file is available to download from the Create Canopy portal
10. Status moves to **Delivered**

### Canopy Staff Side

1. New request appears in the Create Canopy staff queue
...
Create Canopy formalizes the design fee process:
...
At MVP, pricing and payment processing are handled outside the portal (Canopy invoices through their normal billing process). Create Canopy records the quoted amount and approval — the billing record — without requiring an integrated payment system at this stage.

Payment integration is a later capability.

## File Delivery and Version Management

Every design request in Create Canopy maintains a full version history. This is especially important for recurring publications like class catalogs, which are updated every term, and for any project that goes through multiple revision rounds.
...
For recurring publications like class catalogs, Create Canopy tracks versions across requests over time — not just within a single project:
...
Finished design files are stored per workspace in Create Canopy. School staff can:
...
Create Canopy sits at the start of the content production pipeline:

- **Publish Canopy** — finished catalogs and brochures produced through Create Canopy are uploaded to Issuu and assigned to the workspace in Publish Canopy
...
Create Canopy connects to the Canopy platform through the standard model:
...
Create Canopy is primarily a managed service workflow product — school staff request, Canopy produces. There is no self-serve design tool inside Canopy at MVP.
...
7. The portal dashboard card for Create Canopy shows active requests and a "New Request" action
